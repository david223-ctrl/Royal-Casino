...
function claimBonus(bonusName) {
    alert("Успешно! " + bonusName + " теперь в вашем профиле.");
}
...
// Закрыть меню, если пользователь выбрал пункт (для удобства)
...
// Этот код выполняется сразу, как только загружается любая страница
document.addEventListener('DOMContentLoaded', function() {
    const username = localStorage.getItem('username');
    const authButtons = document.getElementById('auth-buttons'); // Убедитесь, что у контейнера кнопок такой ID
    
    if (username && authButtons) {
        // Если имя есть в памяти, меняем кнопки на профиль
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (username) {
        // Если пользователь залогинен, скрываем кнопки входа и показываем профиль
...
// Добавьте эту функцию для кнопки "Выйти"
...
window.addEventListener('load', () => {
    ...
    const username = localStorage.getItem('username');
    console.log("Имя пользователя в памяти:", username);
    
    if (username) {
        const authButtons = document.getElementById('auth-buttons');
        const userProfile = document.getElementById('user-profile');
        
        console.log("authButtons найден:", !!authButtons);
        console.log("userProfile найден:", !!userProfile);
        
        if (authButtons && userProfile) {
            authButtons.style.display = 'none';
            userProfile.style.display = 'flex';
            userProfile.querySelector('#profile-username').textContent = username;
        }
    }
});
...
// Чтобы меню закрывалось, если нажали на ссылку в меню
document.querySelectorAll('.nav-menu a').forEach(link => {
    link.addEventListener('click', () => {
        document.getElementById('navMenu').classList.remove('active');
    });
